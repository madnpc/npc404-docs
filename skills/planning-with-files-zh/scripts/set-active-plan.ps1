...
#
# Usage:
#   .\set-active-plan.ps1 <plan_id>   — pin docs/.planning\.active_plan to plan_id
...
if (-not (Test-Path $PlanDir)) {
    ...
    Write-Error "Run: init-session.sh `"$PlanId`" to create it, or check docs/.planning\ for available plans."
    exit 1
}

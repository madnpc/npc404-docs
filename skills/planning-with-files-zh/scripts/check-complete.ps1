# 检查 docs/task_plan.md 中所有阶段是否完成
# 始终以退出码 0 结束 — 使用标准输出报告状态
# 由 Stop 钩子调用以报告任务完成状态

param(
    [string]$PlanFile = "docs/task_plan.md"
)

if (-not (Test-Path $PlanFile)) {
    Write-Host '[planning-with-files] 未找到 docs/task_plan.md — 没有进行中的规划会话。'
    exit 0
}
...
if ($COMPLETE -eq $TOTAL -and $TOTAL -gt 0) {
    Write-Host ('[planning-with-files] 所有阶段已完成（' + $COMPLETE + '/' + $TOTAL + '）。如果用户有额外工作，请在开始前于 docs/task_plan.md 中新增阶段。')
} else {
    Write-Host ('[planning-with-files] 任务进行中（' + $COMPLETE + '/' + $TOTAL + ' 个阶段已完成）。停止前请更新 docs/progress.md。')
    if ($IN_PROGRESS -gt 0) {

...
PLAN_FILE="${1:-docs/task_plan.md}"

if [ ! -f "$PLAN_FILE" ]; then
    echo "[planning-with-files] 未找到 docs/task_plan.md — 没有进行中的规划会话。"
    exit 0
fi
...
if [ "$COMPLETE" -eq "$TOTAL" ] && [ "$TOTAL" -gt 0 ]; then
    echo "[planning-with-files] 所有阶段已完成（$COMPLETE/$TOTAL）。如果用户有额外工作，请在开始前于 docs/task_plan.md 中新增阶段。"
else
    echo "[planning-with-files] 任务进行中（$COMPLETE/$TOTAL 个阶段已完成）。停止前请更新 docs/progress.md。"

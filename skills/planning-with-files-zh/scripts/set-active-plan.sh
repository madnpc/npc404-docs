...
#
# Usage:
#   set-active-plan.sh <plan_id>   — pin docs/.planning/.active_plan to plan_id
#   set-active-plan.sh             — print the current active plan (if any)
#
# The active plan is stored in docs/.planning/.active_plan and is read by
...
if [ ! -d "${PLAN_DIR}" ]; then
    ...
    echo "Run: init-session.sh \"${PLAN_ID}\" to create it, or check docs/.planning/ for available plans." >&2
    exit 1
fi

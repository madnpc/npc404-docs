...
#   1. $env:PLAN_ID -> .\docs/.planning\$PLAN_ID\
#   2. .\docs/.planning\.active_plan content
#   3. Newest .\docs/.planning\<dir>\ by LastWriteTime
#   4. Empty (legacy fallback to .\task_plan.md handled by caller)

param(
    [string]$PlanRoot = (Join-Path (Get-Location) "docs/.planning")
)
